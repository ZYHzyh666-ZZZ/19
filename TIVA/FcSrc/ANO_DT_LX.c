...
{
	//=====检测CMD是否返回了校验
//	CK_Back_Check();//交换前的检测
//	//=====检测是否触发发送
//	Check_To_Send(0x30);//GPS数据
//	Check_To_Send(0x33);//速度传感器XYZ方向
//	Check_To_Send(0x34);//角度测距传感器
//	Check_To_Send(0x40);//遥控器数据
//	Check_To_Send(0x41);//控制目标数据
//	Check_To_Send(0xe0);//CMD命令帧
//	Check_To_Send(0xe2);//  X
//	Check_To_Send(0x0d);//电流电压数据
}

//===================================================================


u8 ANO_DT_data_to_send[100];
u8 ANO_DT_data_to_char[100];

void ANO_DT_SendString(u8 color , const char *str)//发送字符串
{
	u8 _cnt=0;
	ANO_DT_data_to_send[_cnt++]=0xAA;
	ANO_DT_data_to_send[_cnt++]=0XFF;//上位机地址
	ANO_DT_data_to_send[_cnt++]=0xA0;//功能码ID
	ANO_DT_data_to_send[_cnt++]=0;
	ANO_DT_data_to_send[_cnt++]=color;
	
	u8 _cnt2=0;
	ANO_DT_data_to_char[_cnt2++]=0xAA;
	
	u8 i = 0;
	while(*(str+i) != '\0')
	{
		ANO_DT_data_to_send[_cnt++] = *(str+i);
		ANO_DT_data_to_char[_cnt2++]= *(str+i++);
		if(_cnt > 50)
			break;
	}
	
	ANO_DT_data_to_char[_cnt2++]= 0XCD;
	ANO_DT_data_to_char[_cnt2++]= 0XCD;
	
	ANO_DT_data_to_send[3] = _cnt-4;//数据长度
	
	u8 check_sum1 = 0, check_sum2 = 0;
	for(u8 i=0;i<_cnt;i++)
	{
		check_sum1 += ANO_DT_data_to_send[i];
		check_sum2 += check_sum1;
	}
	ANO_DT_data_to_send[_cnt++] = check_sum1;
	ANO_DT_data_to_send[_cnt++] = check_sum2;

	ANO_DT_LX_Send_Data(ANO_DT_data_to_send, _cnt);
	
}


void ANO_DT_Send_MY_DATA_1(void)//发送灵活帧  2号
{
	u8 _cnt=0;

	ANO_DT_data_to_send[_cnt++] = 0xAB;
	ANO_DT_data_to_send[_cnt++] = 0X02;//源地址
	ANO_DT_data_to_send[_cnt++] = 0XFF;//目标地址
	ANO_DT_data_to_send[_cnt++] = 0xF1;//功能码ID
	ANO_DT_data_to_send[_cnt++] = 1;
	ANO_DT_data_to_send[_cnt++] = (int)(MY_N100.AHRS.Roll*100);
	ANO_DT_data_to_send[_cnt++] = (int)(MY_N100.AHRS.Roll*100) >> 8;
	ANO_DT_data_to_send[_cnt++] = (int)(MY_N100.AHRS.Pitch*100);
	ANO_DT_data_to_send[_cnt++] = (int)(MY_N100.AHRS.Pitch*100) >> 8;
	ANO_DT_data_to_send[_cnt++] = (int)(MY_N100.AHRS.Yaw*100);
	ANO_DT_data_to_send[_cnt++] = (int)(MY_N100.AHRS.Yaw*100) >> 8;
	...
	ANO_DT_LX_Send_Data(ANO_DT_data_to_send, _cnt);
	//TIVA_Send(data_to_char,_cnt2);//发送给tiva
}
